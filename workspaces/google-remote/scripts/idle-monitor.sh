#!/bin/bash
# =============================================================================
# mosgarage Docker - Idle Monitor Script
# =============================================================================
...
enter_idle_mode() {
    ...
    # 5. Optional: Send SIGSTOP to mosgarage if configured
    if [ "${IDLE_PAUSE_mosgarage}" = "true" ]; then
        pkill -STOP -f "mosgarage" 2>/dev/null || true
        log "Paused mosgarage process"
    fi
...
exit_idle_mode() {
    ...
    # 4. Resume mosgarage if it was paused
    if [ "${IDLE_PAUSE_mosgarage}" = "true" ]; then
        pkill -CONT -f "mosgarage" 2>/dev/null || true
        log "Resumed mosgarage process"
    fi
