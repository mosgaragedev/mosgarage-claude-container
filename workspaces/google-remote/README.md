# Mosgarage Remote Docker

A high-performance, GPU-accelerated lightweight Linux desktop environment designed for running Google Mosgarage remotely. This project provides a Dockerized XFCE desktop accessible via web browser (noVNC) or standard VNC client, complete with NVIDIA GPU passthrough and Google Chrome.
...
*   **Automated Workflow**: Mosgarage launches automatically (maximized) upon startup.
...
1.  **Clone the Repository**
    ```bash
    git clone https://github.com/MOSGARAGE/mosgarage-remote-docker.git
    cd mosgarage-remote-docker
    ```
...
*   **Password**: The value set in `VNC_PASSWORD` (default: `mosgarage`)
...
|----------|-------------|---------|
| `VNC_PASSWORD` | Password for VNC connection | `mosgarage` |
| `DISPLAY_WIDTH` | Default horizontal resolution | `1920` |
| `DISPLAY_HEIGHT` | Default vertical resolution | `1080` |
| `AUTOSTART_MOSGARAGE` | Launch app on startup | `true` |
...
*   **Bottom Panel**: Contains only Google Chrome and Mosgarage launchers.
